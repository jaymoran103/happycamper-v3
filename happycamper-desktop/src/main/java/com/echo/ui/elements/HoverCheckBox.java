...
/**
 * HoverCheckBox extends JCheckBox to add a hover effect and enter click functionality
 */
public class HoverCheckBox extends JCheckBox {

    private final boolean DO_ENTER_LISTENER = true;
     
    /**
     * Standard constructor passes on given text, Calls init.
     * @param text String to display, like any JCheckBox
     * @param state Setting state to represent
     */
    public HoverCheckBox(String text,Boolean state){
        super(text,state);
        init();
    }
     ...
     * @param text String to display, like any JCheckBox
     */
    public HoverCheckBox(String text){
        super(text);
        init();
    }
    ...
    /**
     * Adds a keyListener, clicking the checkbox when Enter is pressed and the checkbox is focused.
     */
